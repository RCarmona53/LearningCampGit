# Let's start with the concept, sans metaprogramming magic. 
# Write a constructor for ErrorMessage which accepts a named hash for initializing @message, @status, and @suggestion.
...
# Fill out the Not class below, and use it to return an object capable of inverting calls to Object#not. 
# We'll use calls to smith? as examples and in the tests. You'll see why in the next exercise.
class Object
  def not
    Not.new(self)
  end
...
# Ooooohhhh, snap. See it now? Not responds to nil?. That's going to be a problem for us. 
# No instance of Not will ever be nil, by definition. 
# That means we need to trap messages sent to Not objects for methods defined on Object itself so they're handled by our inversion logic
